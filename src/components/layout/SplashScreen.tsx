...
export default function SplashScreen() {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-y-0 left-0 w-[calc(50%+1px)] bg-[#0A1D15] border-r border-white/5 z-20 flex items-center justify-end overflow-hidden transition-transform duration-[650ms] ease-[cubic-bezier(0.77,0,0.175,1)] will-change-transform ${
      ...
      <div
        className={`absolute inset-y-0 right-0 w-[calc(50%+1px)] bg-[#0A1D15] border-l border-white/5 z-20 flex items-center justify-start overflow-hidden transition-transform duration-[650ms] ease-[cubic-bezier(0.77,0,0.175,1)] will-change-transform ${
          isExiting ? "translate-x-full" : "translate-x-0"
        }`}
        style={{
          transform: isExiting ? "translate3d(100%, 0, 0)" : "translate3d(0, 0, 0)",
      ...
      >
        ...
        <div className="animate-splash-content relative select-none text-center max-w-full">
          <h1 className="font-display font-black text-5xl sm:text-7xl md:text-8xl lg:text-[14vw] tracking-[0.04em] uppercase text-white leading-none drop-shadow-[0_10px_35px_rgba(0,0,0,0.8)]">
            HEAVEN
          </h1>
        </div>
